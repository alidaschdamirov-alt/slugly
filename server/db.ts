import { eq, and, desc, sql, inArray, like, or, gte, lte, isNull, count, asc } from "drizzle-orm";
import { drizzle } from "drizzle-orm/mysql2";
import { InsertUser, users, projects, links, clicks, domains, retiredCodes, reports, blockedDomains, siteSettings, auditLog, workspaces, workspaceMembers, rateLimits, notifications, notificationRecipients } from "../drizzle/schema";
import type { InsertProject, InsertLink, InsertClick, InsertReport, InsertAuditLogEntry, InsertNotification, InsertNotificationRecipient } from "../drizzle/schema";
...
export async function deleteProjectCascade(id: number) {
  const db = await getDb();
  if (!db) return;
  // Get all links in this project (including short codes for retirement)
  ...
  if (linkIds.length > 0) {
    ...
    // Retire all short codes so they cannot be reused
...
export async function ensureSystemProject(workspaceId: number, userId: number): Promise<number> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  // Check if system project already exists for this workspace
  const existing = await db.select({ id: projects.id }).from(projects)
    .where(and(eq(projects.workspaceId, workspaceId), eq(projects.isSystem, true)))
    .limit(1);
...
export async function getLinksByUserId(userId: number, filters?: {
  ...
  if (filters?.projectId) {
    conditions.push(eq(links.projectId, filters.projectId));
  }
  if (filters?.status) {
    conditions.push(eq(links.status, filters.status));
  }
  if (filters?.search) {
    ...
    conditions.push(
      or(
        ...
        like(links.destinationUrl, searchTerm),
        like(links.title, searchTerm)
      )!
    );
  }

  const result = await db.select().from(links).where(and(...conditions)).orderBy(desc(links.createdAt));
  return result;
}
...
export async function getLinksByTag(userId: number, tag: string) {
  const db = await getDb();
  if (!db) return [];
  // JSON_CONTAINS checks if the tags array contains the given tag
  return db.select().from(links)
    .where(and(eq(links.userId, userId), sql`JSON_CONTAINS(${links.tags}, JSON_QUOTE(${tag}))`))
    .orderBy(desc(links.createdAt));
}
...
export async function deleteLink(id: number) {
  const db = await getDb();
  if (!db) return;
  // Get the short code before deleting so we can retire it
  ...
  // Retire the short code so it can never be reused
  if (link[0]?.shortCode) {
...
export async function getAllTagsByUserId(userId: number): Promise<Array<{ tag: string; linkCount: number }>> {
  ...
  return Object.entries(tagMap)
    .map(([tag, linkCount]) => ({ tag, linkCount }))
    .sort((a, b) => b.linkCount - a.linkCount);
}

// Get click stats for links with a specific tag
export async function getTagClickStats(userId: number, tag: string, days: number = 30) {
  ...
  const tagLinks = await db.select({ id: links.id }).from(links)
    .where(and(eq(links.userId, userId), sql`JSON_CONTAINS(${links.tags}, JSON_QUOTE(${tag}))`));
  ...
  const [totalResult, timeResult, topLinksResult, countriesResult, devicesResult, referrersResult] = await Promise.all([
    ...
    db.select({
      day: sql<string>`DATE(FROM_UNIXTIME(timestamp / 1000))`,
      count: sql<number>`COUNT(*)`
    }).from(clicks)
      .where(and(inArray(clicks.linkId, linkIds), gte(clicks.timestamp, since)))
      .groupBy(sql`DATE(FROM_UNIXTIME(timestamp / 1000))`)
      .orderBy(sql`DATE(FROM_UNIXTIME(timestamp / 1000))`),
    
    db.select({
      linkId: clicks.linkId,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(inArray(clicks.linkId, linkIds)).groupBy(clicks.linkId).orderBy(desc(sql`COUNT(*)`)).limit(10),

    db.select({
      value: clicks.country,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(inArray(clicks.linkId, linkIds), sql`${clicks.country} IS NOT NULL`)).groupBy(clicks.country).orderBy(desc(sql`COUNT(*)`)).limit(10),

    db.select({
      value: clicks.deviceType,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(inArray(clicks.linkId, linkIds), sql`${clicks.deviceType} IS NOT NULL`)).groupBy(clicks.deviceType).orderBy(desc(sql`COUNT(*)`)).limit(10),

    db.select({
      value: clicks.referrer,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(inArray(clicks.linkId, linkIds), sql`${clicks.referrer} IS NOT NULL AND ${clicks.referrer} != ''`)).groupBy(clicks.referrer).orderBy(desc(sql`COUNT(*)`)).limit(10),
  ]);
...
export async function getClickCountsByLinkIds(linkIds: number[]) {
  ...
  const result = await db.select({
    linkId: clicks.linkId,
    count: sql<number>`COUNT(*)`
  }).from(clicks).where(inArray(clicks.linkId, linkIds)).groupBy(clicks.linkId);
  
  const map: Record<number, number> = {};
  for (const row of result) {
    map[row.linkId] = row.count;
  }
  return map;
}
...
export async function getClicksOverTime(linkId: number, days: number = 30) {
  ...
  const result = await db.select({
    ...
    .where(and(eq(clicks.linkId, linkId), gte(clicks.timestamp, since)))
    .groupBy(sql`DATE(FROM_UNIXTIME(timestamp / 1000))`)
    .orderBy(sql`DATE(FROM_UNIXTIME(timestamp / 1000))`);
  return result;
}
...
export async function getClicksOverTimeForLinks(linkIds: number[], days: number = 7) {
  ...
  const result = await db.select({
    ...
    .where(and(inArray(clicks.linkId, linkIds), gte(clicks.timestamp, since)))
    .groupBy(clicks.linkId, sql`DATE(FROM_UNIXTIME(timestamp / 1000))`)
    .orderBy(sql`DATE(FROM_UNIXTIME(timestamp / 1000))`);
...
export async function getClickStats(linkId: number) {
  ...
  const [countriesResult, devicesResult, browsersResult, referrersResult] = await Promise.all([
    db.select({
      value: clicks.country,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(eq(clicks.linkId, linkId), sql`${clicks.country} IS NOT NULL`)).groupBy(clicks.country).orderBy(desc(sql`COUNT(*)`)).limit(10),
    
    db.select({
      value: clicks.deviceType,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(eq(clicks.linkId, linkId), sql`${clicks.deviceType} IS NOT NULL`)).groupBy(clicks.deviceType).orderBy(desc(sql`COUNT(*)`)).limit(10),
    
    db.select({
      value: clicks.browser,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(eq(clicks.linkId, linkId), sql`${clicks.browser} IS NOT NULL`)).groupBy(clicks.browser).orderBy(desc(sql`COUNT(*)`)).limit(10),
    
    db.select({
      value: clicks.referrer,
      count: sql<number>`COUNT(*)`
    }).from(clicks).where(and(eq(clicks.linkId, linkId), sql`${clicks.referrer} IS NOT NULL AND ${clicks.referrer} != ''`)).groupBy(clicks.referrer).orderBy(desc(sql`COUNT(*)`)).limit(10),
  ]);
...
export async function getProjectClickStats(projectId: number, days: number = 30) {
  ...
  const [totalResult, uniqueResult, timeResult, topLinksResult] = await Promise.all([
    db.select({ count: sql<number>`COUNT(*)` }).from(clicks)
      .where(and(inArray(clicks.linkId, linkIds), gte(clicks.timestamp, since), notBot)),
    
    db.select({ count: sql<number>`COUNT(DISTINCT ${clicks.ipHash})` }).from(clicks)
      .where(and(inArray(clicks.linkId, linkIds), gte(clicks.timestamp, since), notBot)),
    ...
    db.select({
      ...
      .where(and(inArray(clicks.linkId, linkIds), gte(clicks.timestamp, since), notBot))
      .groupBy(clicks.linkId).orderBy(desc(sql`COUNT(*)`)).limit(10),
  ]);
...
export async function getReports(status?: "pending" | "reviewed" | "actioned" | "dismissed") {
  ...
  const baseQuery = db.select({
    ...
    linkId: links.id,
    userId: links.userId,
    destinationDomain: links.destinationUrl,
  }).from(reports)
    .leftJoin(links, eq(links.shortCode, reports.shortCode))
    .orderBy(desc(reports.createdAt));
  if (status) {
    return baseQuery.where(eq(reports.status, status));
  }
  return baseQuery;
}
...
export async function getClicksForExport(linkId: number, days?: number) {
  ...
  if (days) {
    const since = Date.now() - days * 24 * 60 * 60 * 1000;
    conditions.push(gte(clicks.timestamp, since));
  }
  // Get link shortCode + UTM fields for the export
  ...
  }).from(links).where(eq(links.id, linkId)).limit(1);
  ...
  }).from(clicks).where(and(...conditions)).orderBy(desc(clicks.timestamp)).limit(10000);

  return rows.map(r => ({
    "Date": new Date(Number(r.timestamp)).toISOString().replace("T", " ").slice(0, 16),
    "Short URL": shortCode,
    ...
    "OS": r.os || "",
    "Referrer": r.referrer || "",
    "UTM Source": linkData?.utmSource || "",
    "UTM Medium": linkData?.utmMedium || "",
    "UTM Campaign": linkData?.utmCampaign || "",
    "UTM Term": linkData?.utmTerm || "",
    "UTM Content": linkData?.utmContent || "",
  }));
}
...
export async function getProjectClicksForExport(projectId: number, days?: number) {
  ...
  const projectLinks = await db.select({
    ...
    utmTerm: links.utmTerm,
    utmContent: links.utmContent,
  }).from(links).where(eq(links.projectId, projectId));
  ...
  const clickRows = await db.select({
    ...
    os: clicks.os,
    referrer: clicks.referrer,
  }).from(clicks).where(and(...conditions)).orderBy(desc(clicks.timestamp)).limit(50000);
  
  return clickRows.map(c => ({
    "Date": new Date(Number(c.timestamp)).toISOString().replace("T", " ").slice(0, 16),
    "Short URL": linkMap[c.linkId]?.shortCode || "",
    "Destination": linkMap[c.linkId]?.destinationUrl || "",
    "Project": projectName,
    "Country": c.country || "",
    "Device": c.deviceType || "",
    "Browser": c.browser || "",
    "OS": c.os || "",
    "Referrer": c.referrer || "",
    "UTM Source": linkMap[c.linkId]?.utmSource || "",
    "UTM Medium": linkMap[c.linkId]?.utmMedium || "",
    "UTM Campaign": linkMap[c.linkId]?.utmCampaign || "",
    "UTM Term": linkMap[c.linkId]?.utmTerm || "",
    "UTM Content": linkMap[c.linkId]?.utmContent || "",
  }));
}
...
export async function exportUserData(userId: number) {
  ...
  let userClicks: any[] = [];
  if (linkIds.length > 0) {
    userClicks = await db.select().from(clicks).where(inArray(clicks.linkId, linkIds)).limit(100000);
  }
...
export async function deleteUserAccount(userId: number) {
  ...
  if (linkIds.length > 0) {
    await db.delete(clicks).where(inArray(clicks.linkId, linkIds));
    // Retire all short codes
    for (const l of userLinks) {
      await db.insert(retiredCodes).values({ shortCode: l.shortCode }).onDuplicateKeyUpdate({ set: { shortCode: l.shortCode } });
    }
    await db.delete(links).where(inArray(links.id, linkIds));
  }
...
export async function adminSearchLinks(query: string) {
  ...
  return db.select().from(links)
    .where(or(like(links.shortCode, searchTerm), like(links.destinationUrl, searchTerm), like(links.title, searchTerm)))
    .orderBy(desc(links.createdAt))
    .limit(50);
}
...
export async function adminSearchUsers(query: string) {
  ...
  if (!db) return [];
  const searchTerm = `%${query}%`;
  return db.select({ id: users.id, name: users.name, email: users.email, role: users.role, plan: users.plan, createdAt: users.createdAt })
    .from(users)
...
export async function claimAnonymousLinks(shortCodes: string[], userId: number) {
  const db = await getDb();
  if (!db) return;
  // Only claim links that have no owner (userId = 0 or null)
  for (const code of shortCodes) {
    await db.update(links)
      .set({ userId, expiresAt: null }) // Remove TTL on claim
      .where(and(eq(links.shortCode, code), or(eq(links.userId, 0), isNull(links.userId))));
  }
}
...
export async function adminGetUserById(userId: number) {
  ...
  // Get violation history from audit log
  const violations = await db.select().from(auditLog)
    .where(and(eq(auditLog.targetType, "user"), eq(auditLog.targetId, String(userId))))
...
export async function adminDeleteUser(userId: number) {
  const db = await getDb();
  if (!db) return;
  // Get all links to retire their codes
  const userLinks = await db.select({ id: links.id, shortCode: links.shortCode }).from(links).where(eq(links.userId, userId));
  for (const link of userLinks) {
    await db.insert(retiredCodes).values({ shortCode: link.shortCode });
  }
  // Delete in order: clicks → links → projects → domains → user
  const linkIds = userLinks.map(l => l.id);
  if (linkIds.length > 0) {
    await db.delete(clicks).where(inArray(clicks.linkId, linkIds));
  }
...
export async function adminSearchLinksAdvanced(opts: { query?: string; ownerId?: number; status?: string; anonymous?: boolean }) {
  ...
  if (opts.status) conditions.push(eq(links.status, opts.status as any));
  ...
  const query = db.select({
    ...
    .orderBy(desc(links.createdAt)).limit(100);
  if (conditions.length > 0) return query.where(and(...conditions));
  return query;
}
...
export async function adminCleanupExpiredAnonymous() {
  ...
  if (!db) return 0;
  const now = Date.now();
  // Find expired anonymous links
  const expired = await db.select({ id: links.id, shortCode: links.shortCode })
    .from(links)
    .where(and(eq(links.userId, 0), lte(links.expiresAt, now)));
...
export async function adminGetMetrics() {
  ...
  // Registrations today
  const [regsTodayResult] = await db.select({ count: count() }).from(users)
    .where(gte(users.createdAt, new Date(oneDayAgo)));
  
  // Registrations this week
  const [regsWeekResult] = await db.select({ count: count() }).from(users)
    .where(gte(users.createdAt, new Date(oneWeekAgo)));
  
  // Pro users (paid)
  const [proUsersResult] = await db.select({ count: count() }).from(users)
    .where(eq(users.plan, "pro"));
  
  // Subscription statuses
  const [activeSubsResult] = await db.select({ count: count() }).from(users)
    .where(eq(users.subscriptionStatus, "active"));
  const [trialingSubsResult] = await db.select({ count: count() }).from(users)
    .where(eq(users.subscriptionStatus, "trialing"));
  const [pastDueSubsResult] = await db.select({ count: count() }).from(users)
    .where(eq(users.subscriptionStatus, "past_due"));
  const [canceledSubsResult] = await db.select({ count: count() }).from(users)
  ...
  // Active links
  const [activeLinksResult] = await db.select({ count: count() }).from(links)
    .where(eq(links.status, "active"));
  
  // Clicks today
  const [clicksTodayResult] = await db.select({ count: count() }).from(clicks)
    .where(gte(clicks.timestamp, oneDayAgo));
  
  // Clicks this week
  const [clicksWeekResult] = await db.select({ count: count() }).from(clicks)
    .where(gte(clicks.timestamp, oneWeekAgo));
  
  // Open reports
  const [openReportsResult] = await db.select({ count: count() }).from(reports)
    .where(eq(reports.status, "pending"));
  
  // Links disabled today (from audit log)
  const [disabledTodayResult] = await db.select({ count: count() }).from(auditLog)
    .where(and(
      eq(auditLog.action, "link.disable"),
      gte(auditLog.createdAt, new Date(oneDayAgo))
    ));
  
  // Suspended users
  const [suspendedUsersResult] = await db.select({ count: count() }).from(users)
...
export async function setPlanLimits(plan: "free" | "starter" | "pro" | "team", limits: {
  ...
  if (limits.features) {
    Object.assign(configs[plan].features, limits.features);
  }
...
export async function getUnreadNotificationCount(userId: number) {
  ...
  const [result] = await db.select({ count: sql<number>`COUNT(*)` })
    .from(notificationRecipients)
    .where(and(eq(notificationRecipients.userId, userId), eq(notificationRecipients.read, false)));
...
export async function markNotificationRead(recipientId: number, userId: number) {
  ...
  await db.update(notificationRecipients)
    ...
    .where(and(eq(notificationRecipients.id, recipientId), eq(notificationRecipients.userId, userId)));
}
  ...
  switch (audience.type) {
    ...
    case "plan": {
      const rows = await db.select({ id: users.id }).from(users)
        .where(and(eq(users.plan, audience.value as any), eq(users.suspended, false)));
      return rows.map(r => r.id);
    }
    case "role": {
      const rows = await db.select({ id: users.id }).from(users)
        .where(and(eq(users.role, audience.value as any), eq(users.suspended, false)));
      return rows.map(r => r.id);
    }
    case "workspace": {
      const wsId = parseInt(audience.value || "0");
      const rows = await db.select({ userId: workspaceMembers.userId }).from(workspaceMembers)
