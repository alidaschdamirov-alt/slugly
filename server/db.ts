...
async function assertUserIsNotProtectedAdmin(
  ...
  if (isProtectedAdminEmail(target?.email)) {
    throw new Error(`Protected administrator cannot be ${action}`);
  }
}
...
export async function deleteProjectCascade(id: number) {
  ...
  if (linkIds.length > 0) {
    ...
    for (const pl of projectLinks) {
      await db
        .insert(retiredCodes)
        .values({ shortCode: pl.shortCode })
        .onDuplicateKeyUpdate({ set: { shortCode: pl.shortCode } });
    }
  }
  // Delete the project
  await db.delete(projects).where(eq(projects.id, id));
...
export async function ensureSystemProject(
  ...
  const existing = await db
    ...
    .from(projects)
    .where(
      and(eq(projects.workspaceId, workspaceId), eq(projects.isSystem, true))
    )
    .limit(1);
...
export async function moveProjectLinks(
  projectId: number,
...
export async function deleteLink(id: number) {
  ...
  if (link[0]?.shortCode) {
    await db
      .insert(retiredCodes)
      .values({ shortCode: link[0].shortCode })
      .onDuplicateKeyUpdate({ set: { shortCode: link[0].shortCode } });
  }
}
...
export async function isShortCodeRetired(shortCode: string): Promise<boolean> {
  ...
  if (!db) return false;
  const result = await db
    .select({ id: retiredCodes.id })
    .from(retiredCodes)
    .where(eq(retiredCodes.shortCode, shortCode))
    .limit(1);
...
export async function getAllTagsByUserId(
  userId: number
): Promise<Array<{ tag: string; linkCount: number }>> {
  const db = await getDb();
  if (!db) return [];
...
export async function getTagClickStats(
  userId: number,
  tag: string,
  days: number = 30
) {
  ...
  const [
    ...
    db
      ...
      .from(clicks)
      .where(
        and(inArray(clicks.linkId, linkIds), sql`${clicks.country} IS NOT NULL`)
      )
    ...
    db
      ...
      .where(
        and(
          ...
          sql`${clicks.deviceType} IS NOT NULL`
        )
      )
    ...
    db
      ...
      .where(
        and(
          inArray(clicks.linkId, linkIds),
          sql`${clicks.referrer} IS NOT NULL AND ${clicks.referrer} != ''`
        )
      )
...
export async function getClicksOverTimeForLinks(
  linkIds: number[],
  days: number = 7
) {
  const db = await getDb();
...
export async function getProjectSparkline(
  linkIds: number[],
  days: number = 7
...
export async function getClickStats(linkId: number) {
  ...
  const [countriesResult, devicesResult, browsersResult, referrersResult] =
    await Promise.all([
      db
        .select({
          value: clicks.country,
        ...
        .from(clicks)
        .where(
          and(eq(clicks.linkId, linkId), sql`${clicks.country} IS NOT NULL`)
        )
        .groupBy(clicks.country)
        .orderBy(desc(sql`COUNT(*)`))
        .limit(10),
      ...
      db
        ...
        .from(clicks)
        .where(
          and(eq(clicks.linkId, linkId), sql`${clicks.deviceType} IS NOT NULL`)
        )
        .groupBy(clicks.deviceType)
        .orderBy(desc(sql`COUNT(*)`))
        .limit(10),
      ...
      db
        ...
        .from(clicks)
        .where(
          and(eq(clicks.linkId, linkId), sql`${clicks.browser} IS NOT NULL`)
        )
...
export async function getProjectClickStats(
  projectId: number,
  days: number = 30
) {
  const db = await getDb();
  ...
  const [totalResult, uniqueResult, timeResult, topLinksResult] =
    await Promise.all([
      db
        .select({ count: sql<number>`COUNT(*)` })
        .from(clicks)
...
export async function addBlockedDomain(hostname: string, reason?: string) {
  ...
  await db
    ...
    .values({ hostname: hostname.toLowerCase(), reason })
...
export async function isHostnameBlocked(hostname: string): Promise<boolean> {
  ...
  const result = await db
    ...
    .from(blockedDomains)
    .where(eq(blockedDomains.hostname, hostname.toLowerCase()))
    .limit(1);
  return result.length > 0;
}
...
export async function getClickCountByLinkIdFiltered(
  linkId: number,
...
export async function getClicksOverTimeFiltered(
  linkId: number,
  days: number = 30,
  excludeBots: boolean = true
) {
...
export async function getClicksForExport(linkId: number, days?: number) {
  ...
  return rows.map(r => ({
    Date: new Date(Number(r.timestamp))
      .toISOString()
      .replace("T", " ")
      .slice(0, 16),
    "Short URL": shortCode,
...
export async function getProjectClicksForExport(
  projectId: number,
  days?: number
) {
  const db = await getDb();
  ...
  return clickRows.map(c => ({
    Date: new Date(Number(c.timestamp))
      .toISOString()
      .replace("T", " ")
      .slice(0, 16),
...
export async function deleteUserAccount(userId: number) {
  ...
  if (linkIds.length > 0) {
    ...
    for (const l of userLinks) {
      await db
        .insert(retiredCodes)
        .values({ shortCode: l.shortCode })
        .onDuplicateKeyUpdate({ set: { shortCode: l.shortCode } });
    }
    await db.delete(links).where(inArray(links.id, linkIds));
  }
...
export async function getTagClicksForExport(
  userId: number,
  tag: string,
  days?: number
) {
...
export async function claimAnonymousLinks(
  ...
  for (const code of shortCodes) {
    await db
      ...
      .where(
        and(
          eq(links.shortCode, code),
          or(eq(links.userId, 0), isNull(links.userId))
        )
      );
  }
}
  ...
  const violations = await db
    ...
    .where(
      and(
        ...
        eq(auditLog.targetId, String(userId))
      )
    )
    .orderBy(desc(auditLog.createdAt))
    .limit(20);
...
export async function adminSetRole(
  ...
  if (!db) return;
  if (role !== "admin") {
    await assertUserIsNotProtectedAdmin(db, userId, "demoted");
  }
...
export async function adminSearchLinksAdvanced(opts: {
  query?: string;
  ownerId?: number;
  status?: string;
  anonymous?: boolean;
  ...
  if (opts.query) {
    ...
    conditions.push(
      or(
        like(links.shortCode, term),
        like(links.destinationUrl, term),
        like(links.title, term)
      )
    );
  }
...
export async function adminGetMetrics() {
  ...
  // Registrations today
  const [regsTodayResult] = await db
    .select({ count: count() })
    .from(users)
    .where(gte(users.createdAt, new Date(oneDayAgo)));

  // Registrations this week
  const [regsWeekResult] = await db
    .select({ count: count() })
    .from(users)
  ...
  // Clicks this week
  const [clicksWeekResult] = await db
    .select({ count: count() })
    .from(clicks)
    .where(gte(clicks.timestamp, oneWeekAgo));

  // Open reports
  const [openReportsResult] = await db
    .select({ count: count() })
    .from(reports)
    .where(eq(reports.status, "pending"));

  // Links disabled today (from audit log)
  const [disabledTodayResult] = await db
    .select({ count: count() })
    .from(auditLog)
    .where(
      and(
...
export async function setPlanLimits(
  ...
  if (limits.domains !== undefined)
    configs[plan].limits.domains = limits.domains;
  if (limits.seats !== undefined) configs[plan].limits.seats = limits.seats;
  if (limits.analyticsRetentionDays !== undefined)
    configs[plan].limits.analyticsRetentionDays = limits.analyticsRetentionDays;
  if (limits.features) {
...
export async function adminGetAllUsersEnriched(opts?: {
  search?: string;
  plan?: string;
  suspended?: boolean;
}) {
  ...
  if (opts?.suspended !== undefined)
    conditions.push(eq(users.suspended, opts.suspended));
...
export async function getUserIdsByAudience(audience: {
  type: string;
  value?: string;
  userIds?: number[];
}): Promise<number[]> {
  ...
  switch (audience.type) {
    ...
    case "plan": {
      const rows = await db
        ...
        .from(users)
        .where(
          and(eq(users.plan, audience.value as any), eq(users.suspended, false))
        );
      return rows.map(r => r.id);
    }
    case "role": {
      const rows = await db
        ...
        .from(users)
        .where(
          and(eq(users.role, audience.value as any), eq(users.suspended, false))
        );
      return rows.map(r => r.id);
    }
    case "workspace": {
